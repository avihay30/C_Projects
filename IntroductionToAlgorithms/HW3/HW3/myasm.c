// get user input or name of <name>.asm
// create fixed table for C-instractions
// in c: without = (equal sign) --> dest = 000
// strcmp for checking against table

// when checking symboles:
//		if symbol is in table:
//			translate it;
//		else:
//			add to table (key = name,
//						  value = address [start with 16]
// 
// (Note: in table: keys are uniqe, values arn't)
//
// for running: run compiler as 
//					run in V
//					cmd: <name>.exe <name1>.asm
//				for comperition: check using Assembler of Nand2Tetris
...
void getComp(char* source, char* dest) {
	int i = 0, j = 0;
	while (source[i] != '=' && i < INS_FIELD_MAX_SIZE)
		i++;
	// there is no equal sign -> reseting i
	if (i == INS_FIELD_MAX_SIZE)
		i = -1;
...
// Removes all white spaces (start, end) from string
...
void getOutputFileName(char* fileName, char* outputFile) {
	...
	pToExtention = outputFile + strlen(outputFile) - ASM_EXTETION_LENGTH + 1;
...
int main()
{
	...
	char fileName[FILE_NAME_SIZE + ASM_EXTETION_LENGTH];
	...
	printf("PLEASE NOTE: inputed assembly file and .exe");
	printf("\n\t  should be loacted on the **same** folder\n\n");
